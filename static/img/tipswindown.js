...
function tipsWindown(title,content,width,height,drag,time,showbg,cssName,backcall) {
	...
	var l_w=$('#windown-box').width();
	var lft_w=(w_w-l_w)/2;
	if ( _version == 6.0 ) {
		$("#windown-box").css({left:lft_w,top:(parseInt((ch)/2)+est)+"px",marginTop: -((parseInt(height)+53)/2)+"px",zIndex: "999999"});
	}else {
		$("#windown-box").css({left:lft_w,top:"50%",marginTop:-((parseInt(height)+53)/2)+"px",zIndex: "999999"});
	};
